...
def main():
    try:
        ...
        print(f"[{datetime.now()}] Iniciando extração semanal de REPROVADOS...")
        ...
        if file_size_mb > 25:
            ...
            for i in range(num_parts):
                ...
                send_slack_message(f"weekly_safra_rejected_report.py foi executada com sucesso e o arquivo {part_name} foi gerado.")
            os.remove(temp_path)
        else:
            final_name = f"{base_filename}.csv"
            final_path = os.path.join(output_dir, final_name)
            os.rename(temp_path, final_path)
            send_slack_message(f"weekly_safra_rejected_report.py foi executada com sucesso e o arquivo {final_name} foi gerado.")

        print(f"Linhas totais extraídas: {len(df)}")
        conn.close()

    except Exception as e:
        send_slack_message(f"❌ *ERRO no Relatório Safra Reprovados!*\n- Erro: {str(e)}")
